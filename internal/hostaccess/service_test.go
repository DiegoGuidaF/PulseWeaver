...
type fakeRepo struct {
	createKnownHostFn func(ctx context.Context, fqdn string) (KnownHost, error)
...
func (f *fakeRepo) CreateKnownHost(ctx context.Context, fqdn string) (KnownHost, error) {
	if f.createKnownHostFn != nil {
		return f.createKnownHostFn(ctx, fqdn)
	}
	if f.err != nil {
...
func (f *fakeRepo) CreateHostGroup(_ context.Context, _ string, _ *string) (HostGroup, error) {
	if f.err != nil {
...
func TestService_CreateKnownHost_NotifiesObservers(t *testing.T) {
	...
	_, err := svc.CreateKnownHost(context.Background(), "example.com")
