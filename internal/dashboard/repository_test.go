...
func seedAttributionAggregateRow(t *testing.T, db *database.DB, bucketAt time.Time, kind dashboard.AttributionKind, entityID *int64, entityName string, outcome bool, count int64) {
	t.Helper()
	outcomeInt := 0
	if outcome {
		outcomeInt = 1
...
func attributionByName(counts []dashboard.AttributionCount) map[string]dashboard.AttributionCount {
	m := make(map[string]dashboard.AttributionCount, len(counts))
...
func TestGetAttributionSplit_Policy_RawPath(t *testing.T) {
	...
	entities, err := repo.GetAttributionSplit(ctx, dashboard.AttributionKindPolicy, from, to)
	is.NoErr(err)
...
func TestGetAttributionSplit_Policy_AggregatePath(t *testing.T) {
	...
	policyA := int64(1)
	seedAttributionAggregateRow(t, db, base, dashboard.AttributionKindPolicy, &policyA, "policy-a", true, 5)
	seedAttributionAggregateRow(t, db, base.Add(time.Hour), dashboard.AttributionKindPolicy, &policyA, "policy-a", false, 2)
...
func TestGetAttributionSplit_KindIsolation(t *testing.T) {
	...
	policyID := int64(1)
	userID := int64(1)
	seedAttributionAggregateRow(t, db, base, dashboard.AttributionKindPolicy, &policyID, "policy-a", true, 5)
	seedAttributionAggregateRow(t, db, base, dashboard.AttributionKindUser, &userID, "alice", true, 9)

	entities, err := repo.GetAttributionSplit(ctx, dashboard.AttributionKindUser, base, base.Add(48*time.Hour))
	is.NoErr(err)
...
func TestGetAttributionSplit_DeletedEntity(t *testing.T) {
	...
	base := time.Date(2025, 2, 1, 12, 0, 0, 0, time.UTC)
	seedAttributionAggregateRow(t, db, base, dashboard.AttributionKindDevice, nil, "retired-laptop", true, 4)

	entities, err := repo.GetAttributionSplit(ctx, dashboard.AttributionKindDevice, base, base.Add(48*time.Hour))
	is.NoErr(err)
...
func TestGetAttributionSplit_UnknownKind_Errors(t *testing.T) {
	...
	_, err := repo.GetAttributionSplit(ctx, dashboard.AttributionKind("nonsense"),
...
func TestGetAttributionSplit_SharedIPMultiDeviceDedup(t *testing.T) {
	...
	users, err := repo.GetAttributionSplit(ctx, dashboard.AttributionKindUser, from, to)
	is.NoErr(err)
	...
	devices, err := repo.GetAttributionSplit(ctx, dashboard.AttributionKindDevice, from, to)
	is.NoErr(err)
...
func TestGetAttributionSplit_User_RawAndAggregatePathsAgree(t *testing.T) {
	...
	raw, err := repo.GetAttributionSplit(ctx, dashboard.AttributionKindUser, currentHour.Add(-4*time.Hour), currentHour)
	is.NoErr(err)
	agg, err := repo.GetAttributionSplit(ctx, dashboard.AttributionKindUser, currentHour.Add(-48*time.Hour), currentHour)
	is.NoErr(err)
