...
| `/address-history` | `AddressHistoryPage` | `AddressHistoryView` — TTL-tuning readout + renewal-timing plot over a filterable list of address lease events. |
...
| `address-history` | Address-lease event list + chart, split into an events endpoint (filters+cursor) and a histogram endpoint (filters+window) per ADR-009 §6.4. | `components/{AddressHistoryView,AddressHistoryTable}.tsx`, `addressHistoryColumns.tsx`, `filterConfig.ts` (filterx column config: `device_id`/`user_id`/`ip`/`source`/`event_kind`/`ttl_risk`, each `in`/`not_in`(/`contains`/`not_contains` for `ip`) via the sibling `{field}_op` param); `hooks/use{AddressHistory,AddressHistoryHistogram,AddressHistoryFilters,LocalAddressHistoryFilters}.ts`. `useAddressHistoryFilters` (URL-backed, dedicated page) and `useLocalAddressHistoryFilters` (local-state-backed, device tab) share one `useFilterCore`; a `LockedFilter` (`{key, values}`) pre-applies a non-removable filter and hides its column — the device tab locks `device_id`. `ttl_risk`/`event_kind` render from server enums (`TTL_RISK_BADGE`/`EVENT_KIND_COLORS` in `constants.ts`) — no client-side threshold math. `ttl_risk` is surfaced to the user as **"TTL headroom"** (`TTL_HEADROOM_COLUMN_LABEL`), ranked Comfortable / Tight / Very tight / Past TTL; the wire names stay `ok`/`approaching`/`critical`/`breached`. Its `unknown` value is an absence of a reading, not a severity, so it renders as a dimmed dash with a hint rather than a badge. Columns are user-managed via the shared `useManagedColumns`/`ColumnsMenu`, same as the access log. The histogram's chart half is two presentational leaf components under `AddressHistoryTable`: `AddressHistoryRiskChart.tsx` (stacked device-count bands per bucket over a contiguous, zero-filled series — `ok` is charted as the denominator, and `approaching`+`critical` merge into one "Near TTL" band because four one-hue steps cannot hold adjacent-pair colour separation) and `AddressHistoryTuningStrip.tsx` (the `at_risk_devices` readout: renewal counts, late share, and a suggested TTL snapped onto `lib/ttlPresets`; clicking a name applies the `device_id` filter through the same mechanism as any other column filter, and the device-scoped view keeps the readout but drops that now-redundant link). The palette carries no red — a renewal past its TTL is a tuning signal, not an outage. |
...
| `lib/ttlPresets.ts` | `TTL_PRESET_SECONDS`, `formatTtlLabel`, `suggestTtlSeconds` | the address-lease TTL ladder, shared so a suggested TTL always lands on a value the control offers — `devices/AddressLeaseRuleCard` renders the ladder, `address-history/AddressHistoryTuningStrip` snaps `p95_gap_seconds` onto it |
