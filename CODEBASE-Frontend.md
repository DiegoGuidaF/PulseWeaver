...
suggested lease TTL in from the address-history readout. A `RootLayout` route renders `AuthProvider` + `AppErrorBoundary` around an `<Outlet/>`
...
| `lib/ttlPresets.ts` | `TTL_PRESET_SECONDS`, `formatTtlLabel`, `suggestTtlSeconds`, `SUGGESTED_TTL_PARAM`, `parseSuggestedTtlSeconds` | the address-lease TTL ladder plus the channel a suggestion travels on, shared so a suggested TTL always lands on a value the control offers — `devices/AddressLeaseRuleCard` renders the ladder and stages an incoming `?suggest_ttl=`, `address-history/AddressHistoryTuningStrip` snaps `p95_gap_seconds` onto it. The param is validated through the generated lease-rule zod schema, latched at mount and stripped from the URL, and only ever pre-fills the form — saving stays a user action |
