...
export const getAuditLog = <ThrowOnError extends boolean = false>(options?: Options<GetAuditLogData, ThrowOnError>) => (options?.client ?? client).get<GetAuditLogResponses, GetAuditLogErrors, ThrowOnError>({
    requestValidator: async (data) => await zGetAuditLogData.parseAsync(data),
    responseTransformer: getAuditLogResponseTransformer,
    responseValidator: async (data) => await zGetAuditLogResponse.parseAsync(data),
    security: [{
            in: 'cookie',
            name: '__Host-wdc_session',
            type: 'apiKey'
