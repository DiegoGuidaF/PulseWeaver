...
__How it works:__
WallyDex exports your enrolled IPs into a whitelist file in Caddy snippet format.
The file defines a named snippet `@wallydex_allowlist` with `remote_ip` lines and can be consumed by Caddy via the `import` directive to restrict access to your private services, ensuring only known devices can connect.
...
- **Caddy Integration:** Automatically generates a whitelist file as a Caddy snippet block (`@wallydex_allowlist` with `remote_ip` lines), compatible with Caddy's `import` directive for instant access control.
