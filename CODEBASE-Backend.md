...
| `anomaly` | Periodic detection scan over the access log + traffic aggregates. Detectors are pure readers behind a `Detector` interface; the `ScanJob` owns the incremental watermark and the deduplicated finding upsert. The novelty family additionally reports `ProfileObservation`s via the optional `ProfileLearner` interface, which the job persists to `device_profiles` inside the same scan transaction. Findings persist to `anomalies` for review (no verify-path impact). | `anomaly.go`, `detector.go`, `job.go`, `repository.go`, `reads.go`, `rules.go`, `probing.go`, `volume.go`, `volume_reads.go`, `baseline.go`, `novelty.go`, `novelty_reads.go`, `travel.go` |
